/*  
  Copyright [2016] [zhangwei zhangw428@163.com]
...
SimpleApp.Global = {
		...
		for(var i=0;i<pages.length;i++){
			...
			if(itm.getAttribute('id')){
				...
				if($.inArray(pageId,extArr) == -1){
					if(Global[pageId + 'Controller']){
						if(	$('#' + pageId).length >0 && Global[pageId + 'Controller'].destroy){
							Global[pageId + 'Controller'].destroy(true);
							tempArr.push(pageId);	
						}
					}
				}
	...
	pageSwitch:function(pageIn,pageOut){
		...
		setTimeout(function(){
			...
			if(pages.length > 0){
				...
				self.GC();
				Global[self.idxName+'Controller'].show('reload');
			}
		},400)
	},
	...
	pageSlide:function(pageIn,pageOut){
		...
		var outObj = Global[id_out + 'Controller'];
		var inObj = Global[id_in + 'Controller'];
		var back = false;
		if(!pageOut){
			back = true;	
		}
	...
	checkRoute:function(){
		...
		if(location.href.indexOf('?') != -1 ){  //特定网址
			...
			if(paramObj.pageName){
				...
				if(Global[page+'Controller']){
					var func = function(){
						Global[page+'Controller'].dirShow(paramObj);  //直接从网址导入显示 ，需要可以直接跳转的页面配置dirShow函数	
					}
					func();
				}	
			}else{
...
//添加路由
//isBack 是否是同一个控制器之间的返回如pageH?pageId=1和pageH?pageId=2
SimpleApp.PageController.prototype.handleRoute = function(isBack){
	var self = this;
	if(SimpleApp.Global.routeHistory.length > 0){
	 ...
	 if(!isIn){
		arr.push({'name':self.name,'backRemove':self.backRemove,'backGC':self.backGC,'pageId':self.pageId });
	 }else{
		if(self.circleRoute){
			if(self.name != arr[arr.length -1].name){  //路由上相邻2个不同页面
				arr.push({'name':self.name,'backRemove':self.backRemove,'backGC':self.backGC,'pageId':self.pageId });
			}else{  //路由上相邻2个相同控制器页面
				if(!isBack){
					arr.push({'name':self.name,'backRemove':self.backRemove,'backGC':self.backGC,'pageId':self.pageId });	
	  ...
	  SimpleApp.Global.routeHistory.push({'name':self.name,'backRemove':self.backRemove,'backGC':self.backGC,'pageId':self.pageId }); 
  }
}
...
//callback 页面回调函数
//isBack   是否是同一个控制器之间的返回如pageH?pageId=1和pageH?pageId=2
SimpleApp.PageController.prototype.show = function(isReload ,callback,isBack){
  var self = this;
  ...
  if(pageDom.length == 0 || isReload){
	  ...
	  $.ajax({
		 ...
		 success:function(obj){
			 ...
			 self.handleRoute(isBack);  //添加本页路由
		 }
	  })
  }else{
	 SimpleApp.Global.pageSwitch($('#' + self.name), $(activePage)); 
	...
	if(SimpleApp.Global.routeHistory.length > 0){
		...
		if(page){
			...
			for(var i=idx+1;i<rt.length;i++){  //删除page页面之后的页面和路由
				if(rt[i]){
					var name = rt[i]['name'];
					Global[name + 'Controller'].destroy(true);
				}
			}
		}
		var isBack = '';   //是否是同一个控制器之间的返回如pageH?pageId=1和pageH?pageId=2
		var name = rt[rt.length-1].name;
		var pageId = '';
		if(rt.length > 1){
			if(name == rt[rt.length-1].name){
				isBack = true;
				pageId = rt[rt.length-1].pageId;	
			}
		}
		//console.log(name);
		var pageObj = Global[name + 'Controller'];
		if(pageObj){
			pageObj.show((reload ? reload : ''),function(){
				if(isBack && self.setData){
					self.setData({'pageId':pageId});	
				}
		...
		Global[SimpleApp.Global.idxName+'Controller'].show('reload');	
	}
